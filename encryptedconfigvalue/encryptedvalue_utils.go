...
// stringVarReplacer receives the sequence of bytes that is the content of a string variable (for an input like
// "${...}", the content of the braces) and returns the sequence of bytes that should replace the variable. The bool
// indicates whether the returned bytes should replace the entire variable or just the contents. For example, given
// "${foo}", a replacer that returns ("bar", false) would result in "bar", while a replacer that returns ("bar", true)
// would result in "${bar}".
